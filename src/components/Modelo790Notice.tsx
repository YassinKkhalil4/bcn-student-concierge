...
export function Modelo790Notice() {
  ...
  return (
    <div className="border border-paper-line bg-white p-7">
      <h3 className="text-display-sm text-ink">{t("title")}</h3>
      <p className="mt-3 text-sm leading-relaxed text-ink-muted">{t("intro")}</p>
      <ol className="mt-6 space-y-4">
        {steps.map((step, i) => (
          <li key={step} className="flex gap-4">
            <span className="flex h-6 w-6 flex-none items-center justify-center bg-ink text-[11px] font-semibold text-paper">
              {i + 1}
            </span>
      ...
      <div className="mt-6 bg-paper-dim px-4 py-3.5">
